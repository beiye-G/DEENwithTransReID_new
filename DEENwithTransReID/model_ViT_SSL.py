...
default_cfgs = {
    ...
    'vit_small_patch16_224': _cfg(
        url='https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-weights/vit_small_p16_224- ec54c9.pth',
    ),
...
class TransReID(nn.Module):
    ...
    def __init__(self, class_num, dataset, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0., camera=0, view=0,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, local_feature=False, sie_xishu =1.0, kwargs=kwargs):
    ...
    def forward_features(self, x):
        B = x.shape[0] #x.shape:torch.Size([32, 3, 256, 128])
        x = self.patch_embed(x) #x.shape:torch.Size([32, 465, 768])
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            # 因为SSL Pre-training模型前面的参数有以'base.'开头的，而ViT没有，所以要去掉
...
# def resize_pos_embed_TransReID_SSL(posemb, posemb_new, hight, width):
#     # Rescale the grid of position embeddings when loading from state_dict. Adapted from
#     # https://github.com/google-research/vision_transformer/blob/00883dd691c63a6830751563748663526e811cee/vit_jax/checkpoint.py#L224
#     ntok_new = posemb_new.shape[1]
...
#     print(kwargs.keys())
#     return model
def vit_base_patch16_224_TransReID(class_num, dataset, img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, kwargs=kwargs):
    model = TransReID(
        class_num=class_num, dataset=dataset, img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,\
        camera=camera, view=view, drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),  sie_xishu=sie_xishu, local_feature=local_feature, kwargs=kwargs)
    print(kwargs.keys())
    return model
