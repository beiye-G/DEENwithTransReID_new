...
from model_ViT_SEP_bn_classifier import TransReID_SEP, vit_base_patch16_224_TransReID_SEP, vit_small_patch16_224_TransReID_SEP, deit_small_patch16_224_TransReID_SEP
from utils import *
...
if dataset == 'sysu':
    ...
    train_mode = [1, 2] #1: visible, 2: infrared
...
suffix = dataset
suffix = suffix + '_cmtr_lr_sep_bn_classifier_{}_n{}_lr_{}_seed_{}'.format(args.num_pos, args.batch_size, args.lr, args.seed)
...
if args.optim == 'AdamW':
    # ignored_params =   list(map(id, net.bottleneck.parameters())) \
    #                  + list(map(id, net.classifier.parameters()))
    ignored_params =   list(map(id, net.bottleneck_vis.parameters())) \
                     + list(map(id, net.classifier_vis.parameters())) \
                     + list(map(id, net.bottleneck_inf.parameters())) \
                     + list(map(id, net.classifier_inf.parameters()))
    ...
    optimizer = optim.AdamW([
        {'params': base_params, 'lr': 0.1 * args.lr},
        {'params': net.bottleneck_vis.parameters(), 'lr': args.lr},
        {'params': net.classifier_vis.parameters(), 'lr': args.lr},
        {'params': net.bottleneck_inf.parameters(), 'lr': args.lr},
        {'params': net.classifier_inf.parameters(), 'lr': args.lr}],
        weight_decay=5e-4)
