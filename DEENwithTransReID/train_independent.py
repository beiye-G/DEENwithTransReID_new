...
suffix = dataset
suffix = suffix + '_cmtr_independent_p{}_n{}_lr_{}_seed_{}'.format(args.num_pos, args.batch_size, args.lr, args.seed)
...
net_inf = vit_base_patch16_224_TransReID(n_class, dataset)


# 加载预训练模型的参数
net_vis.load_param('/home/guohangyu/data/VIReID/DEENwithTransReID/DEEN/model/vit_base.pth')
net_vis.to(device)
net_inf.load_param('/home/guohangyu/data/VIReID/DEENwithTransReID/DEEN/model/vit_base.pth')
net_inf.to(device)
...
if len(args.resume) > 0:
    ...
    if os.path.isfile(model_path):
        ...
        checkpoint = torch.load(model_path)
        start_epoch = 0 #checkpoint['epoch']
        net_inf.load_state_dict(checkpoint['net'])
        print('==> loaded checkpoint {} (epoch {})'
              .format(args.resume, checkpoint['epoch']))
    else:
        print('==> no checkpoint found at {}'.format(args.resume))
...
if args.optim == 'AdamW':
    ...
    # inf img
    ignored_params_inf =   list(map(id, net_inf.bottleneck.parameters())) \
                     + list(map(id, net_inf.classifier.parameters()))

    base_params_inf = filter(lambda p: id(p) not in ignored_params_inf, net_inf.parameters())

    optimizer_inf = optim.AdamW([
        {'params': base_params_inf, 'lr': 0.1 * args.lr},
        {'params': net_inf.bottleneck.parameters(), 'lr': args.lr},
        {'params': net_inf.classifier.parameters(), 'lr': args.lr}],
        weight_decay=5e-4)
...
def train(epoch):
    ...
    current_lr_vis = adjust_learning_rate(optimizer_vis, epoch)
    current_lr_inf = adjust_learning_rate(optimizer_inf, epoch)
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        feat1, feat1_att, out1 = net_vis(input1)
        feat2, feat2_att, out2 = net_inf(input2)
        # print(net.training)
        # print(net)
    ...
    writer.add_scalar('lr_vis', current_lr_vis, epoch)
    writer.add_scalar('lr_inf', current_lr_inf, epoch)
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(gall_loader):
            ...
            if test_mode[0] == 1:
                feat, feat_att, out = net_vis(input)
            elif test_mode[0] == 2:
                feat, feat_att, out = net_inf(input)
...
# for epoch in range(start_epoch, 71 - start_epoch):
    ...
    if epoch > 0 and epoch % 2 == 0:
        ...
        if cmc_all[0] > best_acc:  # not the real best for sysu-mm01
            ...
            best_epoch = epoch
            state_vis = {
                'net': net_vis.state_dict(),
                'cmc': cmc_all,
                'mAP': mAP_all,
                'mINP': mINP_all,
                'epoch': epoch,
            }
            torch.save(state_vis, checkpoint_path + suffix + 'vis' + '_best.t')
            state_inf = {
                'net': net_inf.state_dict(),
                'cmc': cmc_all,
                'mAP': mAP_all,
                'mINP': mINP_all,
                'epoch': epoch,
            }
            torch.save(state_inf, checkpoint_path + suffix + 'inf' + '_best.t')
        ...
        if epoch > 10 and epoch % args.save_epoch == 0:
            ...
            torch.save(state_inf, checkpoint_path + suffix + 'vis' +  '_epoch_{}.t'.format(epoch))
            state_inf = {
                'net': net_vis.state_dict(),
                'cmc': cmc_all,
                'mAP': mAP_all,
            ...
            torch.save(state_inf, checkpoint_path + suffix + 'inf' +  '_epoch_{}.t'.format(epoch))
