...
class TransReID(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.local_feature:
            ...
            # 除cls_token外的其它patches的特征
            p = x[:, 1:] #torch.Size([32, 465, 768])
            ...
            p = p.reshape(p.size(0) * p.size(1), -1) #torch.Size([32, 465, 768])
            p = self.bottleneck(p)
            p = p.reshape(p.size(0), p.size(1), -1)  # reshape p back to [32, 465, 768]

            # 返回cls_token 
            return x, x_att, out, p
...
def vit_base_patch16_224_TransReID(class_num, dataset, img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, **kwargs):
    # from train_patches import args
