...
class TransReID(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.patch_embed(x) #x.shape:torch.Size([32, 465, 768])[strid=8时]；x.shape: torch.Size([32, 128, 768])[strid=16时]
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            ...
            if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
                ...
                # v.shape: torch.Size([1, 197, 768]), self.positional_embedding.shape: torch.Size([1, 129, 768])
                # To resize pos embedding when using model at different size from pretrained weights
                if 'distilled' in model_path:
                    print('distill need to choose right cls token in the pth')
                    v = torch.cat([v[:, 0:1], v[:, 2:]], dim=1)
                v = resize_pos_embed(v, self.pos_embed, self.patch_embed.num_y, self.patch_embed.num_x)
                # v: torch.Size([1, 129, 768]), self.patch_embed.num_y: 16, self.patch_embed.num_x: 8
            try:
