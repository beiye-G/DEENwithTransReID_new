...
class TransReID_SEP(nn.Module):
    ...
    #              drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, local_feature=False):
        ...
        #         embed_dim=embed_dim) 
        if hybrid_backbone is not None:
            self.patch_embed_vis = HybridEmbed(
                hybrid_backbone, img_size=img_size, in_chans=in_chans, embed_dim=embed_dim)
            self.patch_embed_inf = HybridEmbed(
                hybrid_backbone, img_size=img_size, in_chans=in_chans, embed_dim=embed_dim)
        else:
            self.patch_embed_vis = PatchEmbed_overlap(
                img_size=img_size, patch_size=patch_size, stride_size=stride_size, in_chans=in_chans,
                embed_dim=embed_dim)
            self.patch_embed_inf = PatchEmbed_overlap(
                img_size=img_size, patch_size=patch_size, stride_size=stride_size, in_chans=in_chans,
                embed_dim=embed_dim)

        # num_patches = self.patch_embed.num_patches
        num_patches = self.patch_embed_vis.num_patches

        # self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.cls_token_vis = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.cls_token_inf = nn.Parameter(torch.zeros(1, 1, embed_dim))
        # self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))
        self.pos_embed_vis = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))
        self.pos_embed_inf = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))
        ...
        self.blocks = nn.ModuleList([
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer)
            for i in range(depth-self.sep_depth)])

        # self.norm = norm_layer(embed_dim)
        # self.norm = norm_layer(embed_dim)
        self.norm_inf = norm_layer(embed_dim)
        self.norm_vis = norm_layer(embed_dim)

        # Classifier head
        # self.fc = nn.Linear(embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        # trunc_normal_(self.cls_token, std=.02)
        trunc_normal_(self.cls_token_vis, std=.02)
        trunc_normal_(self.cls_token_inf, std=.02)
        # trunc_normal_(self.pos_embed, std=.02)
        trunc_normal_(self.pos_embed_vis, std=.02)
        trunc_normal_(self.pos_embed_inf, std=.02)
        ...
        # Bottleneck
        self.bottleneck_vis = nn.BatchNorm1d(pool_dim)
        self.bottleneck_vis.bias.requires_grad_(False)  # no shift
        self.bottleneck_vis.apply(weights_init_kaiming)
        self.bottleneck_inf = nn.BatchNorm1d(pool_dim)
        self.bottleneck_inf.bias.requires_grad_(False)  # no shift
        self.bottleneck_inf.apply(weights_init_kaiming)

        # Classifier
        self.classifier_vis = nn.Linear(pool_dim, class_num, bias=False)
        self.classifier_vis.apply(weights_init_classifier)
        self.classifier_inf = nn.Linear(pool_dim, class_num, bias=False)
        self.classifier_inf.apply(weights_init_classifier)
    ...
    def forward_features(self, x, mode):
        ...
        # visible img
        if mode == 1:
            x = self.patch_embed_vis(x)
            cls_tokens = self.cls_token_vis.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
            x = torch.cat((cls_tokens, x), dim=1)
            x = x + self.pos_embed_vis
        # infrared img
        elif mode == 2:
            x = self.patch_embed_inf(x)
            cls_tokens = self.cls_token_inf.expand(B, -1, -1)
            x = torch.cat((cls_tokens, x), dim=1)
            x = x + self.pos_embed_inf
        ...
        if self.local_feature:
            ...
            if mode == 1:
                x = self.norm_vis(x)
                x = x[:, 0]
                x_att = self.bottleneck_vis(x)
                out = self.classifier_vis(x_att)
            # infrared img
            elif mode == 2:
                x = self.norm_inf(x)
                x = x[:, 0]
                x_att = self.bottleneck_inf(x)
                out = self.classifier_inf(x_att)
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            ...
            if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
                ...
                O, I, H, W = self.patch_embed_vis.proj.weight.shape
                v = v.reshape(O, -1, H, W)
            # elif k == 'pos_embed' and v.shape != self.pos_embed.shape:
            # elif k == 'pos_embed' and v.shape != self.pos_embed_vis.shape:
            elif k == 'pos_embed':
                if  v.shape != self.pos_embed_vis.shape:
                     # To resize pos embedding when using model at different size from pretrained weights
                    if 'distilled' in model_path:
                        print('distill need to choose right cls token in the pth')
                        v = torch.cat([v[:, 0:1], v[:, 2:]], dim=1)
                    # v = resize_pos_embed(v, self.pos_embed, self.patch_embed.num_y, self.patch_embed.num_x)
                    v = resize_pos_embed(v, self.pos_embed_vis, self.patch_embed_vis.num_y, self.patch_embed_vis.num_x)
                k_inf = k.replace('pos_embed', 'pos_embed_inf')
                k_vis = k.replace('pos_embed', 'pos_embed_vis')
                self.state_dict()[k_inf].copy_(v)
                self.state_dict()[k_vis].copy_(v)
                count += 2
                print('Load %d / %d layers.'%(count,len(self.state_dict().keys())))
                # print k，k_inf, k_vis
                print("k:{} k_inf:{} k_vis:{}".format(k, k_inf, k_vis))
                continue
            elif 'blocks' in k:
                match = re.search(r'\d+', k)
                if int(match.group()) >= self.sep_depth:
                    layer = match.group()
                    self.state_dict()[k.replace("."+str(layer), "."+str(int(layer)-self.sep_depth))].copy_(v)
                    count += 1
                if int(match.group()) < self.sep_depth:
                    k_inf = k.replace('blocks', 'inf_blocks')
                    k_vis = k.replace('blocks', 'vis_blocks')
                    self.state_dict()[k_inf].copy_(v)
                    self.state_dict()[k_vis].copy_(v)
                    count += 2
                print('Load %d / %d layers.'%(count,len(self.state_dict().keys())))
                ...
                k_inf = k.replace('patch_embed', 'patch_embed_inf')
                k_vis = k.replace('patch_embed', 'patch_embed_vis')
                self.state_dict()[k_inf].copy_(v)
                self.state_dict()[k_vis].copy_(v)
                count += 2
                print('Load %d / %d layers.'%(count,len(self.state_dict().keys())))
                ...
                k_inf = k.replace('cls_token', 'cls_token_inf')
                k_vis = k.replace('cls_token', 'cls_token_vis')
                self.state_dict()[k_inf].copy_(v)
                self.state_dict()[k_vis].copy_(v)
                count += 2
                print('Load %d / %d layers.'%(count,len(self.state_dict().keys())))
                ...
                k_inf = k.replace('norm', 'norm_inf')
                k_vis = k.replace('norm', 'norm_vis')
                self.state_dict()[k_inf].copy_(v)
                self.state_dict()[k_vis].copy_(v)
                count += 2
                print('Load %d / %d layers.'%(count,len(self.state_dict().keys())))
