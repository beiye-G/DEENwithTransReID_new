...
from model_DEEN_lup import embed_net
from utils import *
...
suffix = suffix + '_deen_lup_p{}_n{}_lr_{}_seed_{}'.format(args.num_pos, args.batch_size, args.lr, args.seed)
...
for epoch in range(start_epoch, 151 - start_epoch):
    ...
    # identity sampler
    # 在 `IdentitySampler` 类中，`__iter__` 方法返回一个迭代器，该迭代器包含所有样本的索引。
    # 在 `data.DataLoader` 中，这个迭代器被用来生成每个批次的样本。
    # 因此，`IdentitySampler` 类的作用是在训练过程中生成每个批次的样本索引，以便 `DataLoader` 可以使用这些索引来生成批次。
