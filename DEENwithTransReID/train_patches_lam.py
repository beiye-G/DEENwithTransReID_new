...
from ViT_patches_lam import TransReID, vit_base_patch16_224_TransReID, vit_small_patch16_224_TransReID, deit_small_patch16_224_TransReID
from utils import *
...
parser.add_argument('--stride', default=16, type=int, help='stride of ViT')
...
suffix = dataset
suffix = suffix + '_patches_lam=1-10_cmtr_stride{}_p{}_n{}_lr_{}_seed_{}'.format(args.stride, args.num_pos, args.batch_size, args.lr, args.seed)
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        feat1, feat1_att, out1, p1 = net(input1)
        feat2, feat2_att, out2, p2 = net(input2)
        # print(net.training)
        # print(net)

        # p1的shape为torch.Size([32, 465, 768])，按照32维度相加，使得最后的shape为torch.Size([465, 768])
        p1 = torch.sum(p1, dim=0)
        # 取P1的平均
        p1 = p1 / input1.size(0)
        p2 = torch.sum(p2, dim=0)
        p2 = p2 / input2.size(0)

        # 计算patches_Loss，按照466维度计算p1和p2的余弦相似度，使得最后的输出结果是465维的
        cos_sim = nn.CosineSimilarity(dim=1, eps=1e-6)
        loss_patches = 1 - cos_sim(p1, p2)
        loss_patches = torch.sum(loss_patches)/loss_patches.size(0)
        ...
        # loss = loss_id + loss_tri + 10 * loss_patches
        loss = loss_id + loss_tri + loss_patches/10
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        # patches_loss.update(loss_patches.item() , 2 * input1.size(0))
        patches_loss.update(loss_patches.item()*10 , 2 * input1.size(0))
        total += labels.size(0)
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                  ...
                  'iLoss:{id_loss.val:.3f} '
                  'TLoss:{tri_loss.val:.3f} '
                  'PLoss:{patches_loss.val:.3f} '.format(
                epoch, batch_idx, len(trainloader),
                train_loss=train_loss, id_loss=id_loss, tri_loss=tri_loss, patches_loss=patches_loss))

    writer.add_scalar('total_loss', train_loss.avg, epoch)
    writer.add_scalar('id_loss', id_loss.avg, epoch)
    writer.add_scalar('tri_loss', tri_loss.avg, epoch)
    writer.add_scalar('patches_loss', patches_loss.avg, epoch)
